...
@section('content')
    <div class="space-y-5 pb-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-1">
            <div>
                <div class="flex items-center gap-1.5 text-xs text-gray-400 mb-1.5">
                    ...
                    <span class="text-gray-700 font-medium">Daftar Mahasiswa</span>
                </div>
                <h1 class="text-[22px] font-semibold text-gray-900 tracking-tight">Daftar Mahasiswa</h1>
                <p class="text-[13px] text-blue-500">Pantau aktivitas unggah dokumen seluruh mahasiswa</p>
            </div>
        ...
        <div class="card px-5 py-4">
            <form method="GET" action="{{ route('kaprodi.users.mahasiswa') }}"
                ...
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama atau email mahasiswa..." class="input-field pl-9">
                </div>

                <div class="relative w-full md:w-36">
                    <span
                        class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 !text-[16px]">filter_list</span>
        ...
        <div class="card overflow-hidden">
            ...
            <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
                ...
                <span class="text-xs text-gray-400">Halaman {{ $mahasiswa->currentPage() }} /
                    {{ $mahasiswa->lastPage() }}</span>
            </div>
                        ...
                        @forelse($mahasiswa as $i => $item)
                            <tr>
                                <td class="text-xs text-gray-300 font-mono">
                                    {{ str_pad($mahasiswa->firstItem() + $i, 2, '0', STR_PAD_LEFT) }}</td>
                                <td>
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <a href="{{ route('kaprodi.users.mahasiswa.show', $item->id) }}"
                                ...
                                <td>
                                    @if ($item->documents->first())
                                        <div class="flex items-center gap-1.5 text-[13px] text-gray-500">
                                            <span
                                                class="material-symbols-outlined !text-[13px] text-gray-300">schedule</span>
                                ...
                                <td class="text-right">
                                    <a href="{{ route('kaprodi.users.mahasiswa.show', $item->id) }}" class="detail-btn">
                                ...
                                <td colspan="7" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        <div
                                            ...
                                            <span
                                                class="material-symbols-outlined text-gray-300 !text-[22px]">group_off</span>
                                        </div>
                                        <div>
                                            <p class="text-sm font-medium text-gray-400">Tidak ada mahasiswa ditemukan</p>
                                            <p class="text-xs text-gray-300 mt-0.5">Coba ubah atau hapus filter pencarian
                                            </p>
                                        </div>
                                        <a href="{{ route('kaprodi.users.mahasiswa') }}"
