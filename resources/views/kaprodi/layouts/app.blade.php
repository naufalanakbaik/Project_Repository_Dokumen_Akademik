...
<body class="bg-slate-50 text-slate-900">
    <div class="min-h-screen flex">
        ...
        <aside class="w-64 bg-white border-r border-slate-200 hidden md:flex flex-col">
            <div class="p-6">
                <h1 class="text-xl font-bold text-indigo-600">RepoDokumen <span class="text-xs font-normal text-slate-400">Kaprodi</span></h1>
            </div>
            
            <nav class="flex-1 px-4 space-y-1">
                <a href="{{ route('kaprodi.dashboard') }}" class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('kaprodi.dashboard') ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50' }} rounded-lg transition-colors">
                    Dashboard
                </a>
        ...
        <main class="flex-1 flex flex-col">
            <header class="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between">
                ...
                <div class="hidden md:block">
                    <h2 class="text-lg font-semibold text-slate-800">@yield('title', 'Kaprodi Dashboard')</h2>
                </div>
