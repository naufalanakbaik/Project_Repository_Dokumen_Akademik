...
@section('content')
    <div class="space-y-5 pb-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-1">
            <div>
                <div class="flex items-center gap-1.5 text-xs text-gray-400 mb-1.5">
                    ...
                    <span class="text-gray-700 font-medium">Aktivitas Pengguna</span>
                </div>
                <h1 class="text-[22px] font-semibold text-gray-900 tracking-tight">Log Aktivitas Pengguna</h1>
                <p class="text-[13px] text-blue-500">Riwayat lengkap aktivitas pengguna dalam sistem repositori</p>
            </div>
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <span
                    class="text-xs text-gray-400 bg-white border border-gray-200 rounded-lg px-3 py-2 flex items-center gap-1.5 shadow-sm">
        ...
        <div class="card px-5 py-4">
            <form method="GET" action="{{ route('kaprodi.activity.index') }}"
                ...
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama pengguna atau judul dokumen..." class="input-field pl-9">
                </div>
                ...
                <div class="relative w-full md:w-48">
                    ...
                    <span
                        class="material-symbols-outlined absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none !text-[16px]">expand_more</span>
                </div>
            ...
            <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3 text-xs text-gray-400">
                    <span class="flex items-center gap-1"><span
                            class="w-2 h-2 rounded-full action-dot-download inline-block"></span>Download</span>
                    <span class="flex items-center gap-1"><span
                            class="w-2 h-2 rounded-full action-dot-preview inline-block"></span>Preview</span>
                        ...
                        @forelse($logs as $i => $log)
                            <tr>
                                <td class="text-xs text-gray-300 font-mono">
                                    {{ str_pad($logs->firstItem() + $i, 2, '0', STR_PAD_LEFT) }}</td>
                                <td>
                                    <div class="flex items-center gap-2.5">
                                        <div
                                            class="w-7 h-7 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center flex-shrink-0">
                                            <span class="text-white text-[10px] font-bold leading-none">
                                                {{ strtoupper(substr($log->user?->name ?? '?', 0, 1)) }}
                                            </span>
                                        </div>
                                        <a href="{{ route('kaprodi.activity.show', $log->id) }}"
                                            class="font-medium text-gray-900 text-[13.5px]">{{ $log->user?->name ?? '—' }}</a>
                                    </div>
                                </td>
                                <td>
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-6 h-6 rounded-md bg-amber-50 border border-amber-100 flex items-center justify-center flex-shrink-0">
                                            <span
                                                class="material-symbols-outlined text-amber-400 !text-[12px]">article</span>
                                        </div>
                                        <span class="text-[13px] text-gray-600 line-clamp-1 max-w-[240px]"
                                ...
                                <td>
                                    <div>
                                        ...
                                        <p class="text-[11px] text-gray-400 mt-0.5">{{ $log->created_at->format('H:i') }} ·
                                ...
                                <td colspan="5" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        <div
                                            ...
                                            <span
                                                class="material-symbols-outlined text-gray-300 !text-[22px]">event_busy</span>
                                        </div>
                                        <div>
                                            ...
                                            @else
                                                <p class="text-xs text-gray-300 mt-0.5">Log aktivitas akan muncul setelah
                                                    ada pengguna yang mengakses dokumen</p>
                                            @endif
                                        </div>
                                    </div>
                                </td>
