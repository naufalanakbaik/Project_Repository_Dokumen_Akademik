...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <form action="{{ route('dosen.documents.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div class="pt-4 flex items-center justify-end space-x-3 border-t border-slate-50">
                <a href="{{ route('dosen.documents.index') }}" class="px-4 py-2 text-slate-600 font-medium hover:text-slate-800 transition-colors">Batal</a>
                <button type="submit" class="px-8 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 font-bold shadow-lg shadow-emerald-200 transition-all transform hover:-translate-y-0.5">
                    Unggah Dokumen
                </button>
            </div>
        </form>
